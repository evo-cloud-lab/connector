...
var Connector = Class(process.EventEmitter, {
    constructor: function (options, logger) {
        ...
        logger || (logger = new Logger('connector:' + this._cluster, '<' + this._id + '> '));
    ...
    /** @field
     * @description Has master capability?
     */
    get mastering () {
        return this._state == 'announcing' || this._state == 'master';
    },
    ...
    start: function () {
        ...
        this.states.start();
        if (this._single) {
            this.logger.notice('SINGLE MODE');
            this.nodes.configure('0.0.0.0', 0);
            this.states.transit('single');
        } else {
    ...
    reconfigure: function (opts, always) {
        ...
        flow.each(reconfigurables).do(function (reconfigurable, next) {
            reconfigurable.reconfigure(options, next);
        }).with(this).run(function (err) {
            if (err) {
                ...
                this.nodes.configure(this.announcer.address, this.announcer.port);
                this.mastering && this.nodes.masterUpdated();
                if (reconnect) {
    ...
    /** @function
     * @description Update local states
     */
    updateLocalStates: function (states) {
        this.nodes.local.updateStates(states);
        this.states.process('localStates');
    },

    /** @function
     * @description Set expectations for all nodes
     */
    setExpectations: function (states) {
        this.states.process('expectations', states);
    },

    /** @function
     * @description Dump connector state
     */
    clusterInfo: function () {
        var data = _.clone(this.mastering ? this.nodes.topology : this.nodes.toObject());
        data.cluster = this.cluster;
        data.state   = this.state;
        data.localId = this.id;
        this._single && (data.single = true);
        return data;
    },

    onNodesUpdated: function () {
        this.logger.notice('NODES %j', this.nodes);
        this.states.process('nodesUpdated');
        this.emit('nodes');
    },
