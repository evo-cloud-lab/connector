...
var Nodes = Class(process.EventEmitter, {
     ...
     *     }
     * }
     * and it will be tranformed and merged into this._allExpects
     * which is
     * {
     *     nodeId1: {
     *         key1: value,
    ...
    set expectations (states) {
        for (var key in states) {
            var nodes = states[key].nodes;
            if (typeof(nodes) == 'object') {
                for (var id in nodes) {
                    var nodeExpects = this._allExpects[id];
                    nodeExpects || (nodeExpects = this._allExpects[id] = {});
                    if (nodes[id] == null) {
                        delete nodeExpects[key];
                    } else {
                        nodeExpects[key] = nodes[id];
                    }
                }
                this._revExpects[key] = states[key].revision;
            }
        }
        this._updated();
    },
