...
var MasterState = Class(State, {
    ...
    onConnection: function (link) {
        ...
        if (link.usage != 'membership') {
            ...
            this.connector.nodes.refresh(link.id, link.address, link.port);
    ...
    onLocalStates: function (states) {
        this.connector.nodes.masterUpdated();
    },

    onExpectations: function (states) {
        this.connector.nodes.expectations = states;
    },
    ...
    'msg:sync': function (msg, link) {
        if (!this.connector.nodes.refresh(link.id, link.address, link.port, msg.data.states) &&
            msg.data.revision != this.connector.nodes.revision) {
            this._refresh(link.id);
        }
    },

    _refresh: function (id) {
        this.connector.linkpool.send({ event: 'refresh', data: this.connector.nodes.topology }, id);
    },
