...
var LinkPool = Class(process.EventEmitter, {
    ...
    /** @function
     * @description Reconfigure listener
     */
    reconfigure: function (options, callback) {
        if (this.receptor) {
            this.receptor.removeAllListeners();
            this.receptor.close();
        ...
        if (this.address && this.port) {
            ...
            callback && process.nextTick(function () {
                callback(new Error('Invalid LinkPool configuration'));
            });
        }
    },
    ...
    _send: function (msg, id) {
        if (id == this.connector.id) {
            // routed to self
            // we only allow msg.event == 'message' to be routed to self
            // the link parameter is a faked object with id and loop flag.
            if (msg.event == 'message') {
                this.emit('message', msg, { id: this.connector.id, loop: true });
            }
        } else {
            var conn = this.connections[id];
            ...
            if (!conn && this.connector.id != this.connector.nodes.masterId) {
                ...
                this.connector.logger.debug('CONNECT %j', info);
